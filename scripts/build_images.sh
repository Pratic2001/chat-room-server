...
log "AI agent search providers (optional — leave blank; keyless Playwright-Google still works)..."
...
# (both required together) add Google web search. Blank all three and the tools
# still work keyless — a headless Chromium (Playwright, baked into the image)
# drives plain Google search and returns real results with no API key. Reuse
# the existing env value as the default so a re-run that didn't change search
# config keeps the same providers — same idempotent behavior as the MAIL_*/OLLAMA_*
# prompts above.
...
# runs. A partial pair just means Google is skipped — the keyless Playwright
# fallback still runs.
if [[ -n "$GOOGLE_API_KEY" && -z "$GOOGLE_CSE_ID" ]] || [[ -z "$GOOGLE_API_KEY" && -n "$GOOGLE_CSE_ID" ]]; then
    warn "GOOGLE_API_KEY and GOOGLE_CSE_ID must both be set for Google search; Google will be skipped (the keyless Playwright fallback still runs)."
fi
